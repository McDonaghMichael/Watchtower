...
function ServerMetricsPage() {
  ...
  if (error) return <div style={{ padding: '20px', color: '#ff4444' }}>Error: {error}</div>;
  if (!metrics || metrics.length === 0) return <div style={{ padding: '20px', color: '#d4d4d4' }}>Loading metrics...</div>;
