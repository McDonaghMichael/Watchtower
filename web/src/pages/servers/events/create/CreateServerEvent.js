...
function CreateServerEvent() {
  ...
  const removeCondition = (conditionId, index) => {
    setConditions((prev) => {
      if (conditionId) {
        return prev.map((c) => (c.condition_id === conditionId ? { ...c, delete: true } : c));
      }
  ...
  const submitData = async (e) => {
    ...
    try {
      const { data: group } = await apiClient.post(`/group`, { server_id: Number(id) });
      ...
      setNotice({ show: true, status: "success", title: "Event created", message: "Conditions and actions were saved successfully." });
    } catch (err) {
      setNotice({
        show: true,
        status: "error",
  ...
  return (
    <Container className="py-4 w-75">
      <Card className="border-0 shadow-sm" style={{ background: "var(--card)", borderRadius: 14, border: "1px solid var(--border)" }}>
        ...
        >
          <div>
            <h4 className="mb-0 fw-semibold" style={{ color: "var(--text)" }}>Add Server Event</h4>
            <small style={{ color: "var(--muted)" }}>Define actions and the conditions that trigger them.</small>
          </div>
            ...
            <section className="mb-4">
              <div className="d-flex justify-content-between align-items-center mb-3">
                <div>
                  ...
                  <small style={{ color: "var(--muted)" }}>What should happen when the rule matches.</small>
                </div>
            ...
            <section className="mb-4">
              <div className="d-flex justify-content-between align-items-center mb-3">
                <div>
                  ...
                  <small style={{ color: "var(--muted)" }}>Chain multiple conditions with AND / OR.</small>
                </div>
                <Button variant="outline-info" size="sm" onClick={addCondition} style={{ borderRadius: 8 }}>
                  <AddBoxIcon fontSize="small" className="me-1" /> Add Condition
                </Button>
